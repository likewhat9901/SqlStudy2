...
- 함수 : 쿼리문의 일부분으로 사용하기 위해 정의하나다. 즉 외부 프로그램에서
    호출하는 경우는 거의 없다.
...
create or replace procedure pcd_emp_salary is
...
/* 데이터 사전에서 확인할 때는 '대문자'로 저장되므로 아래와 같이 upper함수를
사용해야 한다. */
...
*/
--프로시저 생성시 in 파라미터를 설정. first_name 컬럼을 참조하는 형식으로 선언.
...
is
    /* select한 결과를 out파라미터에 저장할 것이므로 별도의 변수가 필요하지 않아
        is절을 비워둔다. */
begin
    /* in 파라미터는 where절의 조건으로 사용하고, select한 결과는
...
붙여야 한다. Out파라미터인 param_salary에 저장된 값이 v_salary로 전달된다. */
...
begin
    ...
    */
    ...
    /*
    행의 변화가 있는 insert, update, delete 쿼리를 실행하는 경우 반드시
...
begin
    gender := '';
    if substr(juminNum, 8, 1) in ('1', '3') then
        gender := '남자';
    elsif substr(juminNum, 8, 1) in ('2', '4') then
        gender := '여자';
    else
        gender := null;
    end if;
...
select substr('999999-100000',8,1) from dual;
select substr('999999-200000',8,1) from dual;
...
create or replace trigger trig_dept_backup
    ...
    /*
    행단위 트리거를 정의. 즉 하나의 행이 변화할 때마다 트리거가 실행된다.
    만약 테이블(문장) 단위 트리거로 정의하고 싶다면 해당 문장을 제거한다.
    이경우 쿼리를 실행하면 트리거도 딱 한번만 실행된다.
    */
    for each row
begin
...
/*
생성2 : 오리지날 테이블에 업데이트 이후 테이블 단위로 발생되는 트리거 생성 */
create or replace trigger trigger_update_test
    after update
    on trigger_dept_original
    /*** for each row -> 이부분 주석 처리 ***/
begin
    if updating then
        dbms_output.put_line ('update 트리거 발생됨');
        /* update 이벤트가 감지되면 백업 테이블에 레코드를 입력 */
