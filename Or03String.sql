...
-- ==> 위 SQL문을 concat() 함수로 변경하면, concat()을 반복해서 써야하므로 불편하다.
select concat('Oracle', '21c', 'Good..!') from dual; --오류.
...
/*
시나리오] 사원테이블에서 first_name 이 john인 사원을 찾아 출력하시오.
*/
...
형식] lpad('문자열', '전체자리수', '채울문자')
    => 전체 자리수에서 문자열의 길이만큼을 제외한 나머지 좌(우)측 부분을
    주어진 문자로 채워준다.('채울문자'가 없으면 빈 공백으로 채운다)
*/
--전체 7글자 중 왼쪽 혹은 오른쪽을 #으로 채운다.
--세번재 인수가 없는 경우에는 공백(스페이스)로 빈공간을 채운다.
...
from dual;
--문자열은 제거할 수 없으므로 에러가 발생한다. (하나의 문자만 가능)
...
select
    ...
    , ltrim(' 좌측공백제거 ', '좌측') --좌측에 공백이 있기 때문에 제거가 안됨.
...
*/
--1.이름의 첫 글자만 출력(인덱스1부터 1글자를 자름)
select first_name, substr(first_name,1,1) from employees;
--2.이름을 10자로 간주하여 나머지 부분을 *로 채워보기
select rpad(first_name,10,'*') from employees;
/*
이름의 첫 글자를 잘라내고, 이름의 길이(length)를 얻어온다. 이를 통해
*를 출력하면 첫 글자를 제외한 남은 길이만큼을 마스킹 처리할 수 있다.
*/
select
    first_name,
...
/*
replace() : 문자열을 다른 문자열로 대체할 때 사용한다. 만약 공백으로 문자열을
    대체한다면 해당 문자열을 삭제하는 결과가 된다. 
...
instr() : 문자열에서 특정문자가 위치한 인덱스를 찾아 반환한다.
    ...
    형식2] instr(컬럼명, '찾을문자', '탐색을 시작할 인덱스', '몇번째문자');
        탐색을 시작할 인덱스부터 문자를 찾는다. 단, 찾는 문자 중 몇번째에 있는
        문자인지 지정할 수 있다.
