...
--커밋을 실행하지 않으면 Java에서 JDBC 작업을 할 수 없다.
commit;
...
insert into board values ('title', 'content', 'id', 'postdate', 1);
...
insert into member (id, pass, name) values ('aaaa', '1234', '테스트');
...
print member_auth;
--아이디만 일치하는 경우
execute MyMemberAuth('spring', 'xxxxx', :member_auth);
print member_auth;
--회원정보가 없는 경우
execute MyMemberAuth('xxxxx', 'yyyyyy', :member_auth);
print member_auth;
