...
begin
    ...
    elsif 로 기술해야 한다. 또한 중괄호 대신 then과 end if를 사용한다. */
    if emp_dept = 50 then
        dept_name := 'Shipping';
...
declare
    ...
    사용하지만, Oracle에서는 그룹함수 sum()이 있으므로 변수명으로 사용할
    수 없다. */
    sumNum number := 0;
begin
    loop
...
declare
    --문자형 변수 생성
    guguDan varchar2(4000) := '';
begin
    for i in 2 .. 9 loop
        for j in 1 .. 9 loop
            --하나의 단씩 문자 변수에 누적해서 연결해준다.
            guguDan := guguDan || i || '*' || j || '=' || i*j || ' ' ;
        end loop;
        --하나의 단이 완성된 문자열을 출력
        dbms_output.put_line(guguDan);
        --출력 후 빈값으로 초기화
        guguDan := '';
    end loop;
end;
...
declare
    ...
    /* 커서선언: 부서테이블의 모든 레코드를 조회하는 select문으로
        into절이 없는 형태로 작성. 쿼리의 실행결과가 cur1에 저장된다. */
    cursor cur1 is
        select
