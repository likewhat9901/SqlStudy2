...
퀴즈] 담당업무 아이디가 ST_MAN인 사원의 사원번호, 이름, 이메일, 매니져아이디를
	...
	뷰명 : emp_st_man_view
*/
create or replace view emp_st_man_view (e_id, name, email, m_id, job_id)
as
    select employee_id, first_name || ' ' || last_name,
        email, manager_id, job_id
...
as
    select employee_id, concat(first_name || ' ', last_name),
    lpad(to_char((salary + (salary * nvl(commission_pct, 0)))*12, '999,999'), 11, ' ')
    from employees ;
select * from v_emp_salary;
----답안----
/*
1.select문 작성. null이 있는 경우 사칙연산이 되지 않으므로 nvl()
함수를 통해 0으로 변환 후 연산식을 작성해야 한다. */
select
    employee_id, last_name,
    (salary + (salary * commission_pct))*12 "1단계",
    (salary + (salary * nvl(commission_pct, 0)))*12 "2단계",
    to_char((salary+(salary*nvl(commission_pct,0)))*12, '999,000') "3단계"
from employees;
--뷰생성
create or replace view v_emp_salary (emp_id, l_name, annual_sal)
as
...
별칭으로 컬럼명을 명시해야 한다. 그렇지 않으면 뷰 생성 시 에러가 발생한다. */
...
as
    select
        ...
        ltrim(to_char((salary+(salary*nvl(commission_pct,0)))*12, '999,000'))
    from employees;
--연산식은 기본컬럼처럼 이름이 있는 것이 아니기 때문에 view에서는 컬럼이름을 지정해야 한다.
create or replace view v_emp_salary2
as
    select
        employee_id, last_name,
...
as
    select
        ...
        state_province || nvl2(state_province, '주의 ', '') || city
    from employees
