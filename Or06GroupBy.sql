...
/*
distinct : 동일한 값이 있는 경우 중복되니 레코드를 제거한 후 하나의 레코드만
    가져와서 보여준다. 순수한 하나의 레코드이므로 통계적인 값을 계산할 수 없다.
*/
select distinct job_id from employees;
/*
group by : 동일한 값이 있는 레코드를 하나의 그룹으로 묶어서 인출한다.
    보여지는 것은 하나의 레코드이지만 다수의 레코드가 하나의 그룹으로 묶여진 결과이므로
    통계적인 값을 계산할 수 있다. 최대, 최소, 평균, 합계 등의 연산이 가능하다. */
...
group by 절이 포함된 select 문의 형식
    ...
    from
        테이블
    where
        조건1 and 조건2 or 조건N <- 물리적으로 존재하는 컬럼이어야 한다.
    group by
        레코드의 그룹화를 위한 컬럼
    having
...
select
    ...
    to_char(sum(salary), '999,999') as sumSalary2,
    trim(to_char(sum(salary), '999,999')) as sumSalary2
from employees;

--10번 부서에서 근무하는 사원들의 급여 합계
select
    trim(to_char(sum(salary), '$999,999')) as sumSalary2
...
select
    ...
    trunc(avg(salary), 2) "평균급여(소수점 2자리)"
from employees;
...
select
    ltrim(to_char(avg(salary), '$999,999.0')) as Avg_Salary
...
각 레코드별로 서로 다른 이름이 저장되어 있으므로 그룹의 조건에 단일 컬럼을 사용할 수 없기
때문이다.
*/
...
select first_name, count(first_name), avg(salary)
    from employees
    group by first_name;
...
select
    department_id "부서번호",
    lpad(count(department_id) || '명', 10, ' ') "사원총합(명)",
    lpad('$ ' || trim(to_char(sum(salary), '999,999')), 10, ' ') "급여총합($)",
    lpad(trim(to_char(avg(salary), '$999,999')), 10, ' ') "평균급여($)"
from employees group by department_id
order by department_id asc;

/*
having : 물리적으로 존재하는 컬럼이 아닌 그룹함수를 통해 논리적으로 생성된 컬럼의
    조건을 추가할 때 사용한다. 해당 조건을 where절에 추가하면 에러가 발생한다.
*/
/*
...
*/
/* 같은 부서에서 근무하더라도 담당업무는 다를 수 있으므로 이 문제에서는 group by절에
2개의 컬럼을 명시해야 한다. 즉, 부서로 그룹화 한 후 다시 담당업무별로 그룹화한다. */
select
    department_id,
    job_id,
    count(*),
...
ex) 급여가 3000인 사원 => 물리적으로 존재하므로 where절에 추가.
    평균급여가 3000인 사원 => 상황에 맞게 논리적으로 만들어낸 결과이므로
                        having 절에 추가해야 한다.
*/
--앞에서 발생된 에러는 having 절로 조건을 옮기면 해결된다.
select
    department_id,
    job_id,
    count(*),
...
select
    job_id,
    to_char(max(salary), '99,999') MaxPay,
    to_char(min(salary), '99,999') MinPay,
    to_char(round(avg(salary)), '99,999') AvgPay,
