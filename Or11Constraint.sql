...
/* 무결성 제약 조건 위배로 에러가 발생된다. PK로 지정된 컬럼 idx에는 중복된 값을
입력할 수 없다. */
insert into tb_primary1 values (2, 'guroError', '오류발생');
--insert문 작성 시 컬럼까지 명시한다.
...
/*PK 지정시 my_pk1로 제약명을 지정했으므로 에러 발생시 해당 이름이 콘솔에 출력된다. */
...
--데이터사전에서 제약조건 확인하기
select * from user_constraints;
--제약조건은 테이블을 대상으로 하므로 테이블 삭제 시 함께 삭제된다.
...
--동시에 설정한 telephone 과 nickname은 동일한 제약명이 부여된다.
select * from user_cons_columns;
...
테이블이 외래키 관계가 있을 때는 부모테이블에 레코드가 있어야 자식테이블에 입력할
수 있다. */
insert into tb_foreign1 values (1, '홍길동', 'gildong');
--아이디 kosmo는 존재하므로 입력성공.
insert into tb_foreign1 values (1, '코스모', 'kosmo');
/* 자식테이블에서 참조하는 레코드가 있으므로, 부모케이블의 레코드를 삭제할
수 없다. 이 경우 반드시 자식테이블의 레코드를 먼저 삭제한 후 부모테이블의
레코드를 삭제해야 한다. */
...
형식은 둘 다 동일하게 외래키를 설정할 때 마지막 부분에 기술하면 된다.
*/
create table tb_primary4 (
    user_id varchar2(30) primary key,
...
단, 참조컬럼의 값이 null로 변경되어, 더이상 참조할 수 없는 레코드가 된다.
*/
...
--test--
create table tb_check3(
    sale_count number not null,
    limit_num number,
        check (sale_count <= limit_num)
);
--제약조건 만족.
insert into tb_check3 values (9, 10);
--제약조건 불만족.
insert into tb_check3 values (11, 10);
select * from tb_check3;
...
alter table pr_emp_const    /* 외래키는 자식테이블에서 생성한다. */
    add constraint pr_emp_dept_fk /* 제약조건명 추가(이름 생략가능) */
