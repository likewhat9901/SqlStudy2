...
--예제1] PL/SQL 맛보기
--화명상에 내용을 출력하고 싶을 때 on으로 설정한다. off면 출력되지 않는다.
set serveroutput on;
...
begin
    ...
    /*
    row_row에는 전체컬럼의 정보가 저장되므로 출력시 변수명.컬러명
    형태로 기술해야 한다. */
...
declare
    /* 인출시 데이터의 변화가 없는 3개의 컬럼은 기존 테이블로 참조해서
...
/*
치환연산자 : PL/SQL에서 사용자로부터 데이터를 입력받을 때 사용하는
...
begin
    for emp_rec in (
        select employee_id as emp_id,
            ...
            nvl(commission_pct, 0) as emp_percent
