...
--1,2는 정상입력
insert into tb_goods values (1, '허니버티칩');
...
/*
시퀀스 생성 후 최초 실행 시에는 currval을 실행할 수 없어 에러가 발생됨.
nextval을 먼저 실행해서 시퀀스를 얻어온 후 실행해야 한다. */
select seq_serial_num.currval from dual;
/*
다음 입력할 시퀀스를 반환한다. 실행할 때마다 설정한 증가치만큼 증가된 값이
반환된다. */
...
/* PK로 지정된 컬럼에 일련번호를 입력하는 용도로 주로 사용되므로 최대값,
싸이클, 캐시는 사용하지 않는것을 권장한다. */
